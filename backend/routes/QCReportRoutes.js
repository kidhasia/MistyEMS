...
import QCReport from '../models/QCReport.js'; // Updated to ES Modules
...
router.get('/:id/download', async (req, res) => {
  try {
    ...
    try {
      await fs.access(logoPath); // Check if logo exists
    ...
    // Pre-summary details
    const { name, assignedTo } = report.taskId || {};
    const assigned = typeof assignedTo === 'string' ? assignedTo : assignedTo?.name;
    const summaryText = `This report summarizes QC findings for Task ${name || 'Unnamed Task'}, including status, remarks, and any follow-up actions required.`;
    ...
    doc.font('Helvetica-Bold')
      ...
      .moveTo(50, tY + 15)
      .lineTo(doc.page.width - 50, tY + 15)
      .stroke('#a501ba');

    const details = [
      ['Task Name:', name || 'Unnamed Task'],
      ['Status:', report.status],
    ...
    doc.moveTo(50, footY)
      .lineTo(doc.page.width - 50, footY)
      .stroke('#a501ba');
    doc.fontSize(8)
      .fillColor('#666')
